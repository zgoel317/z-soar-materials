...
async def test_identity(x):
    ...
    wrapped_fn = process_wrapper(async_fn, preprocess=lambda x: x, postprocess=lambda x: x)
    assert await wrapped_fn(x) == x
...
@pytest.mark.asyncio
async def test_identity_pipeline(get_loader, repeats, convert_into_pipe, max_concurrent):
    ...
    assert set((unflatten(elem) if convert_into_pipe else elem) for elem in await pipeline.run()) == set(elems)
